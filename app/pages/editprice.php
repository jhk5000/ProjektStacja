...
echo '<h2>Edycja ceny</h2><hr class="style-one"></hr>';
if($loged->getGroupId() == 2 || $loged->getGroupId() == 3 || $loged->getGroupId() == 4) {
    if($_GET['id']!=0){
        if($price->getStationsStationId() != $_GET['station']){
            return;
        }
    }
    if($loged->getGroupId() == 2){
        $manager = $entityManager->getRepository('Managers')->findOneBy(array('Users_user_id' => $loged->getUserId()));
        if(!isset($manager)){
            return;
        }else{
            if($manager->getStationsStationId()!=$_GET['station']){
                return;
            }
        }
    }
    ...
    <form action="" method="POST">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="form-group">
                    <label>PB98:</label>
                    <input type="number" step="0.01" min="0.0" class="form-control" name="pb98"
                           value="<?php if ($price) echo $price->getPB98(); ?>"/>
                </div>
                <div class="form-group">
                    <label>PB95:</label>
                    <input type="number" step="0.01" min="0.0" class="form-control" name="pb95"
                           value="<?php if ($price) echo $price->getPB95(); ?>"/>
                </div>
                <div class="form-group">
                    <label>ON:</label>
                    <input type="number" step="0.01" min="0.0" class="form-control" name="on"
                           value="<?php if ($price) echo $price->getOIL(); ?>"/>
                </div>
                <div class="form-group">
                    <label>LPG:</label>
                    <input type="number" step="0.01" min="0.0" class="form-control" name="lpg"
                           value="<?php if ($price) echo $price->getLPG(); ?>"/>
                </div>
                <input type="submit" class="btn btn-primary btn-ls" value="Edytuj"/>
                <a href="<?php if(!isset($manager)){ ?>?page=prices <?php }else{ ?>?page=mystation<?php } ?>">
